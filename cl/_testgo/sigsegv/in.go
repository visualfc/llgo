...
// CHECK: {{^}}@0 = private unnamed_addr constant [4 x i8] c"init", align 1{{$}}
// CHECK: {{^}}@7 = private unnamed_addr constant [7 x i8] c"recover", align 1{{$}}
// CHECK: {{^}}@8 = private unnamed_addr constant [4 x i8] c"main", align 1{{$}}
...
// CHECK-NEXT:   store ptr blockaddress(@"{{.*}}/cl/_testgo/sigsegv.init#1", %_llgo_3), ptr %8, align 8
...
// CHECK-NEXT:   %17 = call ptr @"{{.*}}/cl/_testgo/sigsegv.f"()
// CHECK-NEXT:   %18 = getelementptr inbounds %"{{.*}}/cl/_testgo/sigsegv.T", ptr %17, i32 0, i32 0
// CHECK-NEXT:   %19 = load i64, ptr %18, align 8
// CHECK-NEXT:   call void @"{{.*}}/runtime/internal/runtime.PrintInt"(i64 %19)
// CHECK-NEXT:   call void @"{{.*}}/runtime/internal/runtime.PrintByte"(i8 10)
// CHECK-NEXT:   store ptr blockaddress(@"{{.*}}/cl/_testgo/sigsegv.init#1", %_llgo_6), ptr %9, align 8
// CHECK-NEXT:   br label %_llgo_2
// CHECK-EMPTY:
// CHECK-NEXT: _llgo_5:                                          ; preds = %_llgo_0
// CHECK-NEXT:   store ptr blockaddress(@"{{.*}}/cl/_testgo/sigsegv.init#1", %_llgo_3), ptr %9, align 8
