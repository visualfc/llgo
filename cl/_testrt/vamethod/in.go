...
// CHECK-NEXT:   %8 = insertvalue %"{{.*}}/runtime/internal/runtime.eface" { ptr @"*_llgo_{{.*}}/cl/_testrt/vamethod.CFmt", ptr undef }, ptr %7, 1
// CHECK-NEXT:   %9 = extractvalue %"{{.*}}/runtime/internal/runtime.eface" %8, 0
// CHECK-NEXT:   %10 = call i1 @"{{.*}}/runtime/internal/runtime.Implements"(ptr @"_llgo_{{.*}}/cl/_testrt/vamethod.IFmt", ptr %9)
...
// CHECK-LABEL: define linkonce i32 @__llgo_stub.printf(ptr %0, ptr %1, ...){{.*}} {
// CHECK-NEXT: _llgo_0:
// CHECK-NEXT:   %2 = tail call i32 (ptr, ...) @printf(ptr %1)
