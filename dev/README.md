...
`CHECK` together with the corresponding specific prefix, such as
`DARWIN-ARM64`, `LINUX-AMD64`, or `TARGET-WASM`. Keep portable assertions under
...
autogenerated note used by a plain or pre-ABI fixture. Automatic post-ABI
...
only the differences under target-specific prefixes.
...
- [cl/_testlibc/setjmp/in.go](../cl/_testlibc/setjmp/in.go) demonstrates a handwritten pre-ABI cross-target matrix.
