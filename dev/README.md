...
The plain marker retains the existing IR stage. A handwritten test can
explicitly check the module after target ABI lowering and before LLVM
...
Post-ABI checks enable `CHECK` together with one specific target prefix, for
example `DARWIN-ARM64`, `LINUX-AMD64`, or `TARGET-WASM`. Share portable
assertions under `CHECK` and use the target prefix only on lines that really
