...
func TestWindowsBenchmarkConfiguration(t *testing.T) {
	t.Setenv("LLGO_STDIO_NOBUF", "")

	wantExecutable := filepath.Join("tmp", "program")
	wantNoBuf := false
	if runtime.GOOS == "windows" {
		wantExecutable += ".exe"
		wantNoBuf = true
	}
	if got := nativeExecutable(filepath.Join("tmp", "program")); got != wantExecutable {
		t.Fatalf("nativeExecutable() = %q, want %q", got, wantExecutable)
	}
	if got := nativeExecutable(filepath.Join("tmp", "program.exe")); got != filepath.Join("tmp", "program.exe") {
		t.Fatalf("nativeExecutable() with extension = %q", got)
	}

	hasNoBuf := false
	for _, value := range benchmarkEnv("test-root") {
		if value == "LLGO_STDIO_NOBUF=1" {
			hasNoBuf = true
		}
	}
	if hasNoBuf != wantNoBuf {
		t.Fatalf("benchmarkEnv() has LLGO_STDIO_NOBUF = %v, want %v", hasNoBuf, wantNoBuf)
	}
}
...
func TestRunQuietIncludesCommandOutput(t *testing.T) {
	...
	err = runQuiet(context.Background(), os.Environ(), "definitely-not-an-llgo-command")
	if err == nil || !strings.Contains(err.Error(), "definitely-not-an-llgo-command") {
		t.Fatalf("runQuiet error without command output = %v", err)
	}
}

func TestRunMeasuredIncludesCommandOutputAfterFailure(t *testing.T) {
	err := runMeasured(
		context.Background(),
		os.Environ(),
		"go",
	...
	if err == nil || !strings.Contains(err.Error(), "unknown command") {
		t.Fatalf("runMeasured error = %v", err)
	}
}
