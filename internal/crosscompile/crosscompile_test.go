...
func TestUseWasmLTOCompilesIRToBitcode(t *testing.T) {
	...
	wasi, err := use("wasip1", "wasm", false, false, optlevel.O2, lto.Full, false)
	if err != nil {
		t.Fatal(err)
	}
	if !slices.Contains(wasi.CCFLAGS, "-flto=full") {
		t.Fatalf("wasip1/wasm CCFLAGS missing -flto=full: %v", wasi.CCFLAGS)
	}
	if !slices.Contains(wasi.LDFLAGS, "-flto=full") {
		t.Fatalf("wasip1/wasm LDFLAGS missing -flto=full: %v", wasi.LDFLAGS)
	}
