...
import (
	...
	"github.com/goplus/llgo/runtime/internal/clite/emval"
)
...
func floatValue(f float64) Value {
	...
	return Value{emval.NewDouble(f)}
}
...
func intValue(i int) Value {
	...
	return Value{emval.NewDouble(float64(i))}
}
...
var (
	valueGlobal         = Value{emval.GetGlobal(nil)}
	objectConstructor   = Value{emval.GetGlobal(c.Str("Object"))}
	stringConstructor   = Value{emval.GetGlobal(c.Str("String"))}
	arrayConstructor    = Value{emval.GetGlobal(c.Str("Array"))}
	functionConstructor = Value{emval.GetGlobal(c.Str("Function"))}
)

var (
	valueUndefined = Value{emval.UNDEFINED}
	valueNull      = Value{emval.NULL}
	valueTrue      = Value{emval.TRUE}
	valueFalse     = Value{emval.FALSE}
	valueNaN       = Value{emval.GetGlobal(c.Str("NaN"))}
	valueZero      = Value{emval.NewDouble(0)}
)

// Equal reports whether v and w are equal according to JavaScript's === operator.
func (v Value) Equal(w Value) bool {
	return emval.Equals(v.ref, w.ref) && v.ref != valueNaN.ref
}
...
func stringVal(x string) Value {
	return Value{emval.NewString(c.AllocaCStr(x))}
}
...
func (v Value) Type() Type {
	...
	if emval.IsNumber(v.ref) {
		return TypeNumber
	} else if emval.IsString(v.ref) {
		return TypeString
	}
	switch emval.AsString(emval.Typeof(v.ref)) {
	case "object":
		return TypeObject
	case "symbol":
		return TypeSymbol
...
func (v Value) Get(p string) Value {
	...
	return Value{emval.GetProperty(v.ref, stringVal(p).ref)}
...
func (v Value) Set(p string, x any) {
	...
	// xv := ValueOf(x)
	emval.SetProperty(v.ref, stringVal(p).ref, ValueOf(x).ref)
...
func (v Value) Delete(p string) {
	...
	emval.Delete(v.ref, stringVal(p).ref)
	//valueDelete(v.ref, p)
	//runtime.KeepAlive(v)
}
...
func (v Value) Index(i int) Value {
	...
	return Value{emval.GetProperty(v.ref, intValue(i).ref)}
...
func (v Value) SetIndex(i int, x any) {
	...
	emval.SetProperty(v.ref, intValue(i).ref, ValueOf(x).ref)
	// xv := ValueOf(x)
...
func (v Value) Length() int {
	...
	return Value{emval.GetProperty(v.ref, emval.NewString(c.Str("length")))}.Int()
	//r := valueLength(v.ref)
	//runtime.KeepAlive(v)
	//return r
}
...
func (v Value) Call(m string, args ...any) Value {
	...
	var err c.Int
	if len(args) == 0 {
		res = emval.MethodCall(v.ref, c.AllocaCStr(m), nil, 0, &err)
	} else {
		vargs := make([]emval.Value, len(args))
		for i := 0; i < len(args); i++ {
			vargs[i] = ValueOf(args[i]).ref
		}
		res = emval.MethodCall(v.ref, c.AllocaCStr(m), *(**emval.Value)(unsafe.Pointer(&vargs)), c.Int(len(args)), &err)
	}
	if err != 0 {
...
func (v Value) Invoke(args ...any) Value {
	...
	var err c.Int
	if len(args) == 0 {
		res = emval.Call(v.ref, nil, 0, 0, &err)
	} else {
		vargs := make([]emval.Value, len(args))
		for i := 0; i < len(args); i++ {
			vargs[i] = ValueOf(args[i]).ref
		}
		res = emval.Call(v.ref, *(**emval.Value)(unsafe.Pointer(&vargs)), c.Int(len(args)), 0, &err)
	}
	if err != 0 {
...
func (v Value) New(args ...any) Value {
	...
	var err c.Int
	if len(args) == 0 {
		res = emval.Call(v.ref, nil, 0, 1, &err)
	} else {
		vargs := make([]emval.Value, len(args))
		for i := 0; i < len(args); i++ {
			vargs[i] = ValueOf(args[i]).ref
		}
		res = emval.Call(v.ref, *(**emval.Value)(unsafe.Pointer(&vargs)), c.Int(len(args)), 1, &err)
	}
	if err != 0 {
		if vType := v.Type(); vType != TypeFunction { // check here to avoid overhead in success case
			panic(&ValueError{"Value.Invoke", vType})
...
func (v Value) isNumber() bool {
	return v.ref == valueZero.ref ||
		v.ref == valueNaN.ref ||
		(v.ref != valueUndefined.ref && emval.IsNumber(v.ref))
}
...
func (v Value) InstanceOf(t Value) bool {
	return emval.Instanceof(v.ref, t.ref)
