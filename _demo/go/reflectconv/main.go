...
func MakeRO(v Value) Value {
	(*_Value)(unsafe.Pointer(&v)).flag |= flagStickyRO
	return v
}

// IsRO reports whether v's read-only flag is set.
func IsRO(v Value) bool {
	return (*_Value)(unsafe.Pointer(&v)).flag&flagStickyRO != 0
}
...
func TestConvertFunc(t *testingT) {
	...
	mv := v.Convert(reflect.TypeOf(MyFunc(nil)))
	r := mv.Call([]reflect.Value{reflect.ValueOf(1), reflect.ValueOf(2)})
	if len(r) != 1 || r[0].Int() != 103 {
		t.Errorf("convert func got %v, want %v", r, 103)
	}
}
