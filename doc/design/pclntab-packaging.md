...
- DWARF debug information is generated by default. `-w` omits it.
...
The Go-compatible DWARF selection above is currently aligned for native
Darwin/Linux executable builds. `ModeGen` remains DWARF-free by default and
emits DWARF only when `-w=false` is explicitly requested and supported.
...
part of this policy. Use the default or `-w=false` to retain DWARF, `-w` to
omit it, and `-O0` when a debugger-friendly unoptimized build is required.
...
The affected exact-line, recursive-type, IR-golden, and LTO regression tests
explicitly select `-w` until the corresponding DWARF and PCLN limitations are
fixed. Default-DWARF native integration and the full LLDB suite remain enabled
to protect the currently supported runnable path.
...
IR golden tests explicitly select `-w` when no DWARF policy was requested, so
their `CHECK-NEXT` fixtures continue to describe stable non-debug LLVM IR and
do not depend on the provisional DI implementation. Native integration and
LLDB suites exercise the default-DWARF executable path separately.
